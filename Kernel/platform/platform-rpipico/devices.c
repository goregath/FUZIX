...
bool validdev(uint16_t dev) {
    ...
    if(dev > ((sizeof(dev_tab)/sizeof(struct devsw)) << 8) - 1) return false;
    return true;
}
...
static void timer_tick_cb(unsigned alarm) {
    ...
    if (usbconsole_is_readable()) {
        ...
        vt_inproc(minor(BOOT_TTY), c);
    }
}

void device_init(void) {
    /* The flash device is too small to be useful, and a corrupt flash will cause a crash on startup... oddly. */
    flash_dev_init();
